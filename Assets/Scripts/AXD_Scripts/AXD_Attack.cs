...
public class AXD_Attack : MonoBehaviour
{
    ...
    private int CalculateDamage(AttackType type, bool colateral = false)
    {
        ...
        if (type == AttackType.Swich)
        {
            if (colateral == false)
            {
                totalDamage = Mathf.RoundToInt((playerStats.SwichDamage + (playerStats.SwichDamage * (playerStats.CurrentCombo / 100))) * playerStats.AttackMultiplicatorChain );
            }else if(colateral == true)
            {
                totalDamage = Mathf.RoundToInt(((playerStats.SwichDamage + (playerStats.SwichDamage * (playerStats.CurrentCombo / 100))) * playerStats.AttackMultiplicatorChain)*playerStats.colateralDamage/100);
            }
        }
        else if (type == AttackType.Sponk)
        {
            if (colateral == false) {
                totalDamage = Mathf.RoundToInt((playerStats.SponkDamage + (playerStats.SponkDamage * (playerStats.CurrentCombo / 100))) * playerStats.AttackMultiplicatorChain);
            ...
            {
                totalDamage = Mathf.RoundToInt(((playerStats.SponkDamage + (playerStats.SponkDamage * (playerStats.CurrentCombo / 100))) * playerStats.AttackMultiplicatorChain)*playerStats.colateralDamage/100);
